...
# look at first 100 rows
read_csv(gw_data_csv) %>%
  head(100) %>%
  datatable()
```
...
d = read_csv(gw_data_csv) %>%
  ...
  mutate(
    ...
    year_beg = min(year),
    year_end = max(year),
    gw_score = rescale(lev)*100) %>%  # rescale 0 to 100
  filter(
    year_n   >= 5,                    # at least 5 years of data
    year_end == year(now())) %>%      # last year is this year
  ungroup() %>%
...
gw_w %>%
  ...
  dygraph(main = "Deaths from Lung Disease (UK)") %>%
  dyOptions(
    strokeWidth = 2,
    colors = RColorBrewer::brewer.pal(ncol(gw_w)-1, 'Spectral'))
```
...
labels <- sprintf(
  "<strong>%s</strong> (%d)<br/>groundwater: %g",
  regions$city, regions$region_id, regions$gw_score) %>% 
